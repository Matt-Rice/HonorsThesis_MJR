...
output_dir = "./test_runs/ppo_wall"
os.makedirs(output_dir, exist_ok=True)

# Wall (trained up to rollout 30 because it did well)
...
model = PPO.load('./curriculum_learning/wall/model_rollout_40.zip')
episodes = 10
rewards = []
steps_list = []
trajectories = []
...
for episode in range(episodes):
    ...
    while not done and not truncated:
        ...
        total_reward += reward
        steps += 1
        path_length = info.get("path_length", path_length)  # Update path length if provided in info
...
print(f"Average Reward: {np.mean(rewards):.2f}, Average Steps: {np.mean(steps_list):.2f}, "
      ...
      f"D* Lite Path Length: {path_length}")