...
def train_curriculum(model, train_envs, eval_envs, total_timesteps=100000, save_freq=10000, reward_threshold=200):
    for i, (train_env, eval_env) in enumerate(zip(train_envs, eval_envs)):
        ...
        if i == 1:
            ...
            model = PPO.load("./models/env_0/model_rollout_190.zip", train_env)
            print(f"\nTraining on environment {i + 1}/{len(train_envs)}")
            ...
            print(f"Loading from environment {i-1}")
            print(f"\nTraining on environment {i + 1}/{len(train_envs)}")
            model = PPO.load("./models/env_1/model_rollout_190.zip", train_env)
            print(f"\nTraining on environment {i + 1}/{len(train_envs)}")
            callback = SaveGifAndLogCallback("./logs/", f"./graphs/env_{i}/", f"./models/env_{i}/", eval_env, save_freq)
            model.learn(total_timesteps=100000, callback=callback)
        elif i == 3:
            print(f"Loading from environment {i-1}")
            print(f"\nTraining on environment {i + 1}/{len(train_envs)}")
            model = PPO.load("./models/env_2/model_rollout_190.zip", train_env)
            print(f"\nTraining on environment {i + 1}/{len(train_envs)}")
            callback = SaveGifAndLogCallback("./logs/", f"./graphs/env_{i}/", f"./models/env_{i}/", eval_env, save_freq)
            model.learn(total_timesteps=100000, callback=callback)
        else:
...
if __name__ == "__main__":
    ...
    model = PPO(
        "MlpPolicy", 
        train_envs[0], 
        learning_rate=0.000484401424899623, 
        gamma= 0.9406918495052624,
        batch_size= 32,
        clip_range=0.2314432537575099,
        n_steps=512,
        ent_coef= 0.00011543166467337384,
        verbose=1, 
